...
def ast_dict_update(source, update):
    ...
    if not isinstance(source, ast.Dict):
        raise TypeError("`source` doit être un AST dictionnaire")
    if not isinstance(update, ast.Dict):
        raise TypeError("`update` source` doit être un AST dictionnaire")
    ...
    def inheritance_handler_attributes_update(self, source, specs):
        ...
        for spec in specs:
            ...
            if not isinstance(update_ast, ast.Dict):
                raise TypeError(f"L'opération pour l'attribut `{attr_name}` n'est pas un dictionnaire")
