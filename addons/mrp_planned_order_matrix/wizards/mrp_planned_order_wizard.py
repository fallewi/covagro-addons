...
class MrpPlannedOrderWizard(models.TransientModel):
    ...
    date_end = fields.Date(string="A", required=True)
    date_range_type_id = fields.Many2one(
        string="Type de plage de dates",
        comodel_name="date.range.type",
        required=True,
    )
    product_mrp_area_ids = fields.Many2many(
        string="Paramètres du produit", comodel_name="product.mrp.area", required=True
    )
    ...
    def create_sheet(self):
        ...
        if not self.product_mrp_area_ids:
            raise ValidationError(
                _("Vous devez sélectionner au moins un paramètre Product MRP.")
            )
...
class MprPlannedOrderSheet(models.TransientModel):
    ...
    _description = "Fiche MPS"

    date_start = fields.Date(string="De", readonly=True)
    date_end = fields.Date(string="A", readonly=True)
    date_range_type_id = fields.Many2one(
        string="Type de plage de dates",
        comodel_name="date.range.type",
        readonly=True,
    ...
    product_mrp_area_ids = fields.Many2many(
        string="Paramètres du produit", comodel_name="product.mrp.area"
    )
    ...
    def _onchange_dates(self):
        ...
        if not ranges:
            raise UserError(_("Aucune plage de dates n'a été créée."))
        lines = []
    ...
    def _prepare_planned_order_data(self, line, qty):
        ...
        return {
            "name": "Commande planifiée pour %s"
...
class MprPlannedOrderSheetLine(models.TransientModel):
    ...
    _description = "Ligne de feuille MPS"
    ...
    date_range_id = fields.Many2one(
        ...
        string="Plage de dates",
    )
    value_x = fields.Char(string="Période")
    value_y = fields.Char(string="Produit")
    product_qty = fields.Float(string="Quantité", digits="Unité de quantité de produit")
