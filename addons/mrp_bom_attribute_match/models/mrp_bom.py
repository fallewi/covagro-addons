...
class MrpBomLine(models.Model):
    ...
    product_id = fields.Many2one("product.product", "Composant", required=False)
    product_backup_id = fields.Many2one(
        "product.product", help="Champ technique pour stocker la valeur précédente du champ product_id"
    )
    component_template_id = fields.Many2one(
        "product.template", "Composant (template de produit)"
    )
    match_on_attribute_ids = fields.Many2many(
        "product.attribute",
        string="Correspondance sur les attributs",
