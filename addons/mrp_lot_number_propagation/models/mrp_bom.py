...
class MrpBom(models.Model):
    ...
    lot_number_propagation = fields.Boolean(
        default=False,
        help=(
            "Permettre de propager le numéro de lot/série "
        ...
        for bom in self:
            ...
            if not bom.bom_line_ids.filtered("propagate_lot_number"):
                raise ValidationError(
                    _(
                        "Avec 'Propagation du numéro de lot' activé, une ligne  "
                        "doit être configuré avec l'option 'Propagation du numéro de lot'."
                    )
                )
