...
class MrpProduction(models.Model):
    ...
    is_lot_number_propagated = fields.Boolean(
        ...
        readonly=True,
        help=(
            "Le numéro de lot/série est propagé "
            "d'un composant au produit fini."
        ),
    )
    propagated_lot_producing = fields.Char(
        compute="_compute_propagated_lot_producing",
        help=(
            "La nomenclature utilisée sur cet ordre de fabrication est configurée pour se propager "
            "numéro de lot de l'un de ses composants. La valeur sera "
            "calculé une fois le composant correspondant sélectionné."
        ),
    )
    ...
    def _set_lot_number_propagation_data_from_bom(self):
        ...
        for order in self:
            ...
            if not propagate_move:
                raise UserError(
                    _(
                        "La nomenclature est marquée pour la propagation du numéro de lot, mais"
                        "aucun composant ne propage le numéro de lot. "
                        "Veuillez vérifier la configuration de la nomenclature."
                    )
                )
                ...
                raise UserError(
                    _(
                        "La nomenclature est marquée pour la propagation du numéro de lot, mais "
                        "plusieurs composants propagent le numéro de lot. "
                        "Veuillez vérifier la configuration de la nomenclature."
                    )
                )
            else:
        ...
        for order in self:
            ...
            if finish_moves and not finish_moves.quantity_done:
                ...
                if lot.quant_ids:
                    raise UserError(
                        _(
                            "Le numéro de lot/série %s existe déjà et a été utilisé. "
                            "Impossible de le propager."
                        )
                    )
                if not lot:
    ...
    def write(self, vals):
        for order in self:
            if (
                ...
                raise UserError(
                    _(
                        "Le numéro de lot/série est propagé à partir d'un composant, "
                        "vous n'êtes pas autorisé à le modifier."
                    )
                )
