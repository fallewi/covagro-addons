...
class MrpProductionSerialMatrix(models.TransientModel):
    ...
    _description = "Matrice de série de production Mrp"
    ...
    finished_lot_ids = fields.Many2many(
        string="Numéros de série des produits finis",
        comodel_name="stock.production.lot",
        domain="[('product_id', '=', product_id)]",
    )
    line_ids = fields.One2many(
        string="Cellule matricielle",
    ...
    include_lots = fields.Boolean(
        string="Inclure les lots ?",
        help="Inclure les produits suivis par lots dans la matrice. Suivi des produits par "
        "les numéros de série sont toujours inclus.",
    )
        ...
        for rec in self:
            ...
            for lot, counter in serial_counter.items():
                if counter > 1:
                    warning_lots += lot
                    warning_msgs.append(
                        "Numéro de série %s sélectionné plusieurs fois" % lot.name
                    )
            # Lots
            ...
            if not_filled_lines:
                ...
                warning_msgs.append(
                    "Certaines cellules ne sont pas remplies pour certains numéros de série finis (%s)"
    ...
    def default_get(self, fields):
        ...
        if not production.show_serial_matrix:
            raise UserError(
                _("Le produit fini de ce MO n'est pas suivi par des numéros de série.")
            )
    ...
    def _prepare_matrix_line(self, component_tuple, finished_lot=None, number=None):
        ...
        if finished_lot:
            ...
            res.update(
                {
                    "finished_lot_name": _("(Nouveau Lot %s)") % number,
                }
            )
        return res
    ...
    def button_validate(self):
        ...
        if self.lot_selection_warning_count > 0:
            raise UserError(
                _("Certains problèmes ont été détectés dans votre sélection: %s")
    ...
    def _reserve_lot_in_move(self, move, lot, qty):
        ...
        if (
            ...
            raise ValidationError(
                _("Numéro de série/lot '%s' non disponible à l'emplacement source.") % lot.name
            )
