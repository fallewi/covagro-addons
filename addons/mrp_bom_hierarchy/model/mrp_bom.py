...
class MrpBom(models.Model):
    ...
    def _search_has_child(self, operator, value):
        ...
        if not isinstance(value, bool):
            raise UserError(_("La valeur doit être Vrai ou Faux (pas %s)") % value)
    ...
    def _search_has_parent(self, operator, value):
        if operator not in ["=", "!="]:
            raise UserError(_("Cet opérateur n'est pas pris en charge"))
        if value == "True":
            value = True
        elif value == "False":
            value = False
...
class MrpBomLine(models.Model):
    ...
    has_bom = fields.Boolean(
        string="A une sous-nomenclature",
    ...
    def _bom_line_find_domain(
        ...
        if product:
            ...
            raise UserError(
                _(
                    "Vous devez fournir un produit ou "
                    "un modèle de produit pour rechercher une ligne de nomenclature"
                )
            )
        if picking_type:
            domain += [
