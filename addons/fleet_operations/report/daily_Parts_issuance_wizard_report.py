...
class DailyPartWizard(models.AbstractModel):
    ...
    _description = 'Assurance quotidienne des pièces'
    ...
    def generate_xlsx_report(self, workbook, data, parts_data):
        ...
        row += 1
        row += 1
        worksheet.write(row, 2, 'ÉMISSION QUOTIDIENNE DE PIÈCES', tot)
        row += 1
        worksheet.write(row, 2, 'De:', tot)
        worksheet.write(row, 3, data['form']['date_from'] or '', border)
        worksheet.write(row, 4, 'A:', tot)
        ...
        for objec in self.get_work_order_detail(data['form']):
            row += 3
            worksheet.write(row, 0, 'DATE D\'ÉMISSION:', bold)
            worksheet.write(row, 1, objec.get('date') or '', date)
            row += 2
            worksheet.write(row, 0, 'NO.', format1)
            worksheet.write(row, 1, 'Ordre de Travail NO.', format1)
            worksheet.write(row, 2, 'ID DU VÉHICULE', format1)
            worksheet.write(row, 3, 'ID DE LA PIÈCE.', format1)
            worksheet.write(row, 4, 'NOM DE LA PIÈCE', format1)
            worksheet.write(row, 5, 'MARQUE DU VÉHICULE', format1)
            worksheet.write(row, 6, 'UTILISÉ', format1)
            worksheet.write(row, 7, 'TYPE D\'UNITÉ', format1)
            worksheet.write(row, 8, 'RETOUR PIECE ANCIENNE', format1)
            worksheet.write(row, 9, 'DÉLIVRÉ PAR', format1)
            worksheet.write(row, 10, 'REMARQUES', format1)
            line_row = row + 1
            line_col = 0
            counter = 1
