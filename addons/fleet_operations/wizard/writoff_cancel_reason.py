...
class WizardWritOffCancelReason(models.TransientModel):
    ...
    _description = 'Motif d\'annulation de la radiation du véhicule'

    reason = fields.Char(string='Raison', required=True)
    ...
    def cancel_writoff(self):
        ...
        if self._context.get('active_id', False) and \
            ...
            line = '---------------------------------------------------------'
            line += '--------------------------'
            notes = 'La radiation de votre véhicule est annulée par' + \
                " " + user.name + \
