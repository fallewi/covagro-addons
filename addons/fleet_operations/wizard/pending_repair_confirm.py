...
class PendingRepairConfirm(models.TransientModel):
    ...
    _description = 'En attente de réparation Confirmer'
    ...
    def confirm_wo_forcefully(self):
        ...
        if self._context.get('work_order_id', False):
            ...
            if not increment_ids:
                raise UserError(_("L'odomètre à incrément suivant n'est pas défini pour %s "
                                "veuillez le définir à partir de la configuration !") % work_order.vehicle_id.name)
            if increment_ids:
            ...
            if not next_service_day_ids:
                raise UserError(_("Les prochains jours de service sont "
                                "non configuré pour %s, veuillez le définir à partir de la"
                                "configuration!") % work_order.vehicle_id.name)
            work_order_vals = {}
            if work_order.odometer == 0:
                raise UserError(_("Veuillez définir l' "
                                "Odomètre actuel du véhicule en état de marche !"))
