...
class StockPicking(models.Model):
    ...
    product_kit_helper_ids = fields.One2many(
        ...
        string="Lignes d'assistance pour le kit de produits",
    ...
    has_product_kit = fields.Boolean(
        ...
        help="Vrai s'il y a au moins 1 kit de produits dans la commande client",
    )
...
class StockPickingProductKitHelper(models.Model):
    ...
    picking_id = fields.Many2one(
        ...
        string="sélection des Stocks",
        required=True,
        index=True,
        ondelete="cascade",
    )
    sale_line_id = fields.Many2one(
        comodel_name="sale.order.line", string="Lignes de commande de vente", required=True
    )
    product_id = fields.Many2one(
        comodel_name="product.product", string="Produit", required=True, readonly=True
    )
    ...
    def action_explode_helper(self):
        ...
        for mock_move in mock_processed_moves:
            ...
            if len(stock_move) != 1:
                raise ValidationError(
                    _(
                        "Aucun produit détaillé %(move_display_name)s correspondant pour"
                        " kit de produits %(product_display_name)s"
                    )
                    % {
