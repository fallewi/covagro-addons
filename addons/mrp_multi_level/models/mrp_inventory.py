...
class MrpInventory(models.Model):
    ...
    _description = "Projections d'inventaire MRP"
    ...
    uom_id = fields.Many2one(
        comodel_name="uom.uom", string="Unité du produit", compute="_compute_uom_id"
    )
    date = fields.Date()
    demand_qty = fields.Float(string="Qté Demandé")
    supply_qty = fields.Float(string="Qté Fournie")
    initial_on_hand_qty = fields.Float(
        string="Inventaire de départ", group_operator="avg"
    )
    ...
    running_availability = fields.Float(
        ...
        group_operator="avg",
        help="Niveau de stock théorique si tous les ordres planifiés ont été lancés.",
    )
    ...
    supply_method = fields.Selection(
        string="Méthode d'approvisionnement",
