...
class MultiLevelMrp(models.TransientModel):
    ...
    _description = "Fabrication à plusieurs niveaux"

    mrp_area_ids = fields.Many2many(
        comodel_name="mrp.area",
        string="Zone de Fabrication à exécuter",
        help="Si vide, toutes les zones seront calculées.",
    )
    ...
    def _prepare_mrp_move_data_bom_explosion(
        ...
        if not product_mrp_area:
            raise exceptions.Warning(_("Aucun produit MRP trouvé"))
        factor = (
        ...
        return {
            ...
            "name": (
                "Explosion de la nomenclature de la demande: %s"
                % (name or product.product_id.default_code or product.product_id.name)
            ).replace(
                "Explosion de la nomenclature de la demande: Explosion de la nomenclature de la demande: ", "Explosion de la nomenclature de la demande: "
            ),
    ...
    def explode_action(
        ...
        if not bom:
            return False
        pd = self.env["decimal.precision"].precision_get("Unité de mesure du produit")
    ...
    def _low_level_code_calculation(self):
        ...
        log_msg = "Code bas niveau 0 terminé - Nbr. de produits: %s" % counter
        logger.info(log_msg)
        ...
        while counter:
            ...
            log_msg = "Code bas niveau {} terminé - Nbr. de produits: {}".format(
                llc, counter
            )
            logger.info(log_msg)
            if llc > llc_recursion_limit:
                logger.error("Limite de récursivité atteinte lors du calcul LLC.")
                break

        mrp_lowest_llc = llc
        logger.info("Fin du calcul du code de bas niveau")
    ...
    def _init_mrp_move_grouped_demand(self, nbr_create, product_mrp_area):
        ...
        for move in product_mrp_area.mrp_move_ids:
            ...
            if (
                ...
                name = _(
                    "Demande groupée de %(product_name)s pour %(delta_days)d jours"
                ) % dict(
    ...
    def _mrp_calculation(self, mrp_lowest_llc, mrp_areas):
        ...
        for mrp_area in mrp_areas:
            ...
            log_msg = "Calcul de Fabrication LLC {} Terminé - Nbr. des produits: {}".format(
                llc - 1, counter
            )
            logger.info(log_msg)

        logger.info("Calcul de Fabrication Terminé")
