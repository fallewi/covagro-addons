...
class DateRangeType(models.Model):
    ...
    allow_overlap = fields.Boolean(
        help="Si les ensembles de plages de dates du même type ne doivent pas se chevaucher.", default=False
    )
    active = fields.Boolean(
        help="Le champ actif permet de masquer le type de plage de dates"
        "without removing it.",
        default=True,
    )
    ...
    autogeneration_date_start = fields.Date(
        ...
        help="Ne s'applique que lorsqu'il n'existe pas encore de plages de dates de ce type",
    )
    ...
    _sql_constraints = [
        (
            ...
            "Un type de plage de dates doit être unique par entreprise !",
        )
    ]
            ...
            for rec in self.sudo():
                ...
                if bool(
                    ...
                    raise ValidationError(
                        _(
                            "Vous ne pouvez pas changer d'entreprise, car "
                            "Le type de plage de dates est attribué à la plage de dates"
                            "(%s)."
                        )
    ...
    def autogenerate_ranges(self):
        ...
        for dr_type in self.search(
            ...
            try:
                ...
                logger.warning(
                    "Erreur lors de la génération automatique des plages pour le type de plage de dates "
                    "%s: %s" % (dr_type.name, e)
                )
