...
class DateRange(models.Model):
    ...
    company_id = fields.Many2one(
        comodel_name="res.company", string="Compagnie", index=1, default=_default_company
    )
    active = fields.Boolean(
        help="Le champ actif vous permet de masquer la plage de dates sans"
        "removing it.",
        default=True,
    )
        ...
        for rec in self.sudo():
            if (
                ...
                raise ValidationError(
                    _("%(name)s n'est pas une plage valide (%(date_start)s > %(date_end)s)")
                    % {
                        "name": rec.name,
